...
public class World : Scene
{
    ...
    private bool setCustorToCenter = true;
    ...
    public override void Update()
    {
        ...
        if (Paused)
        {
            ...
            App.WindowGrab = false;

            if (setCustorToCenter)
            {
                Input.Mouse.SetMousePosition(App.Width / 2, App.Height / 2);

                setCustorToCenter = false;
            }
        }
        else
        {
            CameraAngleX = -Utils.Map(Input.Mouse.Position.X, 0, App.Width, 0, MathF.Tau);

            // wrap the mouse inside the window
            if (Input.Mouse.Position.X == App.Width - 1)
            {
                Input.Mouse.SetMousePosition(2, Input.Mouse.Position.Y);
            }
            else if (Input.Mouse.Position.X == 0)
            {
                Input.Mouse.SetMousePosition(App.Width - 2, Input.Mouse.Position.Y);
            }

            App.MouseVisible = false;
            App.WindowGrab = true;

            setCustorToCenter = true;
        }
